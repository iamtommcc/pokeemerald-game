...
static void ShowDecorationOnMap_(u16 mapX, u16 mapY, u8 decWidth, u8 decHeight, u16 decoration)
{
    ...
    for (j = 0; j < decHeight; j++)
    {
        ...
        {
            x = mapX + i;
            attributes = GetMetatileAttributesById(NUM_TILES_IN_PRIMARY_EMERALD + gDecorations[decoration].tiles[j * decWidth + i]);
            ...
            if (elevation != 0xFFFF)
                MapGridSetMetatileEntryAt(x, y, (gDecorations[decoration].tiles[j * decWidth + i] + (NUM_TILES_IN_PRIMARY_EMERALD | overlapsWall)) | impassableFlag | elevation);
            else
                MapGridSetMetatileIdAt(x, y, (gDecorations[decoration].tiles[j * decWidth + i] + (NUM_TILES_IN_PRIMARY_EMERALD | overlapsWall)) | impassableFlag);
        }
    }
}
...
static bool8 CanPlaceDecoration(u8 taskId, const struct Decoration *decoration)
{
    ...
    case DECORPERM_PASS_FLOOR:
        for (i = 0; i < mapY; i++)
        {
            ...
            for (j = 0; j < mapX; j++)
            {
                ...
                layerType = GetMetatileAttributesById(NUM_TILES_IN_PRIMARY_EMERALD + decoration->tiles[(mapY - 1 - i) * mapX + j]) & METATILE_ATTR_LAYER_MASK;
    ...
    case DECORPERM_BEHIND_FLOOR:
        ...
        for (j = 0; j < mapX; j++)
        {
            curX = gTasks[taskId].tCursorX + j;
            behaviorAt = MapGridGetMetatileBehaviorAt(curX, curY);
            layerType = GetMetatileAttributesById(NUM_TILES_IN_PRIMARY_EMERALD + decoration->tiles[j]) & METATILE_ATTR_LAYER_MASK;
